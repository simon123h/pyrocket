...
export class Test {
    ...
    rocket_angle: number = 0;
    rocket_angular_velocity: number = 0;
    sas_mode: string = "land";
    game: RocketGame | null = null;
    stats_before: any = null;
    t_before: number = 0;
    t_after: number = 0;
    stats_after: any = null;
    next_test: Test | null = null;
    ignore_height: boolean = false;

    start(game: RocketGame) {
        console.log("Running test...");
        this.game = game;
        // game.test = this; // Need to add this to RocketGame if we want to follow exactly
        
        const rocket = game.rocket;
        if (!rocket || !rocket.body) return;
    ...
    is_finished(): boolean {
        if (!this.game || !this.game.rocket || !this.game.rocket.body) return false;
        ...
        if (h > 200 && !this.ignore_height) return false;
        if (Math.abs(v.x) > 0.4 || Math.abs(v.y) > 0.4) return false;
