...
export class RocketGame implements IGame {
    ...
    remove_object(obj: GameObject) {
        ...
        if (index !== -1) {
            ...
            if (obj.body) Matter.World.remove(this.world, obj.body);
        }
    }
    ...
    handle_controls() {
        ...
        for (const event of this.events) {
            if (event.type === 'keydown') {
                if (event.code === 'KeyR') this.add_new_rocket();
                if (event.code === 'KeyP') this.run_physics = !this.run_physics;
                if (event.code === 'KeyT') this.run_tests();
                if (event.code === 'KeyA' && this.rocket) this.rocket.airbrakes_enabled = !this.rocket.airbrakes_enabled;
            }
        }
    ...
    length_unit(h: number): string {
        if (Math.abs(h) < 1e3) return `${h.toFixed(1)}m`;
        const ly = 9.461e15;
        if (Math.abs(h) < 0.1 * ly) return `${(h / 1e3).toFixed(1)}km`;
        return `${(h / ly).toFixed(1)}ly`;
    }

    velocity_unit(v: number): string {
        if (Math.abs(v) < 1e3) return `${v.toFixed(1)}m/s`;
